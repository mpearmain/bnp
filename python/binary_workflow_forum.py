...
xgr = xgb.XGBRegressor(n_estimators=g['ne'], max_depth=g['md'], seed=g['rs'], missing=np.nan, learning_rate=0.017, subsample=0.75, colsample_bytree=0.85)
xgc = xgb.XGBClassifier(n_estimators=g['ne'], max_depth=g['md'], seed=g['rs'], missing=np.nan, learning_rate=0.017, subsample=0.75, colsample_bytree=0.85)
...
for c in clf:
    if c[:1] != "x": #not xgb
        model = GridSearchCV(estimator=clf[c], param_grid={}, n_jobs =-1, cv=5, verbose=1, scoring=LL)
    ...
    for i in range(len(y_pred)):
        ...
        if y_pred[i]>1.0:
            y_pred[i] = 1.0
    pd.DataFrame({"ID": id_test, "PredictedProb": y_pred}).to_csv(c + '.csv',index=False)

#Combine the results
id_results = id_test[:]
col1, col2 = ['ID','PredictedProb']
for c in clf:
