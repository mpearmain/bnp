...
if __name__ == "__main__":
	...
	train = pd.read_csv("./train_dvencode_3level2.csv")
	test = pd.read_csv("./test_dvencode_3level2.csv")

	print "Three way encoding.."
	new_var_list = []
	var1_cols = ["v22"]
	var3 = "v66"
	var2_cols = [col for col in cat_columns if col not in var1_cols if col != var3]
	for ind, var1 in enumerate(var1_cols):
                ...
                for var2 in var2_cols:
                        print var1, var2
                        new_var = var1+"_"+var3+"_"+var2
                        train[new_var] = train[[var1,var3,var2]].apply(lambda row: str(row[0])+"_"+str(row[1])+"_"+str(row[2]),axis=1)
                        test[new_var] = test[[var1,var3,var2]].apply(lambda row: str(row[0])+"_"+str(row[1])+"_"+str(row[2]),axis=1)
	...
	train.to_csv("train_dvencode_3level3.csv", index=False)
	test.to_csv("test_dvencode_3level3.csv", index=False)
