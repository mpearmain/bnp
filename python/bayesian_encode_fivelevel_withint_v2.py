...
def getDVEncodeVar(compute_df, target_df, var_name, target_var="target", min_cutoff=5):
        grouped_df = target_df.groupby(var_name, as_index=False)[target_var].agg(["mean", "count"])
...
def prepData(var4_col="v52"):
	...
	for f in cat_columns:
                print f
		new_train = pd.concat([ train[['v1',f, encode_var]], test[['v1',f, encode_var]] ])
		train[encode_var+"Encode_"+str(f)] = getDVEncodeVar(train[["v1", f, encode_var]], new_train[["v1", f, encode_var]], f, target_var=encode_var, min_cutoff=0)
		test[encode_var+"Encode_"+str(f)] = getDVEncodeVar(test[["v1", f, encode_var]], new_train[["v1", f, encode_var]], f, target_var=encode_var, min_cutoff=0)
	...
	other_cols=[]
	new_var_list = []
	for var4_col in ["v52", "v66", "v24", "v56"]: #, "v125", "v30", "v72"]:
	        var1_cols = ["v22"]
		var4 = var4_col
	...
	train.to_csv("train_5levelenc_withint_oob14.csv", index=False)
	test.to_csv("test_5levelenc_withint_oob14.csv", index=False)
...
def prepModel(var4_col="v52"):
        print "Reading files.."
        train = pd.read_csv('./train_4levelenc_withint.csv')
        test = pd.read_csv('./test_4levelenc_withint.csv')
        print train.shape, test.shape
	...
	print "Cross validating.."
        ...
        for fold_index in xrange(1,6):
                ...
                preds, loss = runET(dev_X, dev_y, val_X, val_y, validation=1, n_est_val=500, depth_val=40, split_val=4, leaf_val=2, feat_val=180, jobs_val=4, random_state_val=1122)
                #print feat, loss
        ...
        out_df["et1_srk_4levelenc_withint"] = train_preds
        out_df.to_csv("prval_et1_srk_4levelenc_withint.csv", index=False)

	print "Final model.."
	preds = runET(train, train_y, test, validation=0, n_est_val=500, depth_val=40, split_val=4, leaf_val=2, feat_val=180, jobs_val=4, random_state_val=1122)
	out_df = pd.DataFrame({"ID":test_ID})
        out_df["et1_srk_4levelenc_withint"] = preds
        out_df.to_csv("prfull_et1_srk_4levelenc_withint.csv", index=False)



def prepModelXGB(var4_col="v52"):
        print "Reading files.."
        train = pd.read_csv('./train_5levelenc_withint_oob14.csv')
        test = pd.read_csv('./test_5levelenc_withint_oob14.csv')
        print train.shape, test.shape
	...
	print "Cross validating.."
        ...
        #out_df = pd.DataFrame({"ID":train_ID})
        #out_df["xg1_srk_5levelenc_withint_oob14"] = train_preds
        #out_df.to_csv("prval_xg1_srk_5levelenc_withint_oob14.csv", index=False)
	...
	out_df = pd.DataFrame({"ID":test_ID})
        out_df["xg1_srk_5levelenc_withint_oob14"] = preds
        out_df.to_csv("prfull_xg1_srk_5levelenc_withint_oob14.csv", index=False)
